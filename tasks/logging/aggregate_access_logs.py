...
def aggregate_access_logs():
    """
    Aggrega i log di accesso per ora e giorno della settimana.
    """
    now = timezone.now()
    start_time = now - timezone.timedelta(days=1)
    end_time = start_time + timezone.timedelta(days=1)
    
    logs = AccessLog.objects.filter(timestamp__gte=start_time, timestamp__lt=end_time)
    ...
    with transaction.atomic(using='gold'):
        for entry in aggregation:
            AggregatedAccessLog.objects.update_or_create(
